...
import {
  ...
  SidebarMenuItem,
  SidebarMenuButton,
} from "@/components/ui/sidebar" // <- é o arquivo que você mandou (primitives)
import { LoginDialog } from "@/components/auth/login-dialog" // <- seu dialog corrigido
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <>
        ...
        <SidebarContent>
          <SidebarMenu>
            {items.filter(canSee).map((item) => {
              ...
              // Se estiver bloqueado: ao clicar abre login (não navega)
              if (locked) {
                return (
            ...
            <div className="p-2">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-60">
                  ...
                  <DropdownMenuSeparator />
                  {/* Se quiser no futuro criar rota de conta, troque para Link */}
      ...
      <LoginDialog
        ...
        onSwitchToSignup={() => {
          // Se você tem signup modal, troque aqui.
          alert("Abra o modal de cadastro aqui.")
        }}
        redirectTo={pathname || "/"} // volta pra mesma tela após login
      />
    </>
  )
}
